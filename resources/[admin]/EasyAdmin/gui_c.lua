...
function GenerateMenu() -- this is a big ass function
	...
	menumanagement = _menuPool:AddSubMenu(mainMenu, GetLocalisedText("menumanagement"),"",true, true)
	...
	menumanagement:SetMenuWidthOffset(menuWidth)	
	...
	if permissions["player.unban"] then
		unbanPlayer = _menuPool:AddSubMenu(settingsMenu,GetLocalisedText("viewbanlist"),"",true)
		local thisMenuWidth = menuWidth
		if menuWidth < 150 then
			thisMenuWidth = 150
		else
	...
	menumanagement:AddItem(thisItem)
	thisItem.OnListSelected = function(sender, item, index)
			if item == thisItem then
					i = item:IndexToItem(index)
	...
	if permissions["anon"] then
		...
		menumanagement:AddItem(thisItem)
		menumanagement.OnCheckboxChange = function(sender, item, checked_)
			if item == thisItem then
				anonymous = checked_
